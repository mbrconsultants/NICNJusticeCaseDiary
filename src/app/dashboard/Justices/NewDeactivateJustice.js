...
const CauseListCounsel = () => {
  ...
  //get all qualified justices
  const getAllQualifiedJustices = async (date) => {
    await endpoint
      .get(`/new-justice/qualified-list/${date}`)
      .then((res) => {
        setQualifiedJustices(res.data.data);
        console.log("qualified", res.data.data);
        setLoading(false);
      })
      .catch((err) => {
        setLoading(false);
  ...
  const getAllDisqualifiedJustices = async (date) => {
    await endpoint
      .get(`/new-justice/disqualified-list/${date}`)
      .then((res) => {
        setDisqualifiedJustices(res.data.data);
        console.log("disqualified", res.data.data);
  ...
  const getDateDetails = async (caseId) => {
    await endpoint
      .get(
        `/new-justice/show-disqualification/${deactivateDetails.sitting_date}/${caseId}`
      )
      .then((res) => {
        console.log("meme", res.data.data);
  ...
  const deleteDisqualifiedJustices = (e, id) => {
    ...
    setLoading(true);
    endpoint
      .delete(`/new-justice/delete-disqualification/${id}`)
      .then((res) => {
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      await endpoint
        .post("/new-justice/disqualified", deactivateDetails)
        .then((res) => {
          SuccessAlert(res.data.message);
          history.push(`/justices-disqualifications`);
        })
        .catch((error) => {
  ...
  const handleInputChange = (e) => {
    ...
    const filteredSuggestions = cases.filter((caseItem) =>
      caseItem.suite_no.toLowerCase().includes(value.toLowerCase())
    );
  ...
  const handleSuggestionClick = (suggestion) => {
    getDateDetails(suggestion.case_id.toString());
    setInputValue(suggestion.suite_no);
    setDeactivateDetails({
      ...deactivateDetails,
      case_id: suggestion.case_id.toString(),
    });
            ...
            <div className="card-body">
              ...
              <form
                ...
                <div className="row">
                  ...
                  <div className="col-md-2">
                    <Form.Group>
                      <label htmlFor="exampleInputName1"> Case Category </label>
                      <select
                        className="form-control"
                        onChange={(e) =>
                          setDeactivateDetails({
                            ...deactivateDetails,
                            case_type_cat_id: e.target.value,
                          })
                        }>
                        <option>--select--</option>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setDeactivateDetails({
                            ...
                            qualify_justice_id: e.target.value,
                          })
                        }>
                        <option>--select--</option>
                          ...
                          <td>
                            {data.CaseType
                              ? `${data.CaseType.case_type} (${
                                  data.CaseTypeCategory
                                    ? data.CaseTypeCategory.case_type_cat
                                    : ""
                                })`
                              : ""}
                          </td>
                          <td>
                            {data ? data.DisqualifiedJusticeNew.name : ""}{" "}
                            {data ? data.DisqualifiedJusticeNew.id : ""}
                          </td>

                          <td>
                            {data ? data.QualifiedJusticeNew.name : ""}
                            {data ? data.QualifiedJusticeNew.id : ""}
                          </td>
                          ...
                          <td>
                            <Button
                              ...
                              onClick={() => {
                                setSelectedRow({
                                  id: data.id,
                                  name: data.DisqualifiedJusticeNew.name,
                                });
                                openDeleteModal();
                              }}></Button>
                          </td>
              ...
              <div className="card-body">
                ...
                <form className="forms-sample">
                  <button
                    type="button"
                    onClick={(e) =>
                      deleteDisqualifiedJustices(e, selectedRow.id)
                    }
